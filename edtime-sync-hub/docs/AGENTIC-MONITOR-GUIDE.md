...
cp config/secrets.example.env ~/.edtime-sync/secrets.env
...
```bash
# Dashboard sekali
~/.edtime-sync/scripts/edtime-monitor.sh status
...
| File | Isi |
|------|-----|
| `~/.edtime-sync/edtime/agent-report.json` | Status agent terakhir |
| `~/.edtime-sync/edtime/pending-decisions.json` | **Yang hanya Anda bisa lakukan** |
| `Drive/Edtime/cursor/latest.json` | Data untuk Cursor |
| `~/.edtime-sync/logs/edtime-schedule.log` | Log sync terjadwal |
...
Di Cursor: *"Baca Edtime Sync/cursor/latest.json"*
