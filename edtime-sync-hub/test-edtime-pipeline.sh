...
HUB="${EDTIME_SYNC_HOME:-$HOME/.edtime-sync}"
SCRIPTS="$HUB/scripts"
PASS=0
FAIL=0
...
[[ -f "$REPO_DIR/run-agent.sh" ]] && ok "run-agent.sh" || bad "run-agent"
...
security find-generic-password -s edtime-sync -a webhook-url -w >/dev/null 2>&1 \
  && ok "webhook Keychain" || echo "  ⏭️  webhook Keychain (set via run-agent)"
