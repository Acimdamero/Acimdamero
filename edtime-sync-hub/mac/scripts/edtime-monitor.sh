...
PY
    ...
    echo "  (belum ada — jalankan: bash run-agent.sh)"
  fi
}
...
PY
    ...
    echo "  (belum ada — tunggu sync selesai atau: run-edtime.sh export)"
  fi
}
...
print_daemon() {
  ...
  if [[ "$(uname -s)" == "Darwin" ]]; then
    launchctl list 2>/dev/null | grep -E 'com.edtime.sync' || echo "  (LaunchAgent belum loaded)"
  else
    echo "  (hanya macOS)"
  fi
}
...
case "$cmd" in
  ...
  test)
    echo "── Quick test ──"
    "$HUB_HOME/run-edtime.sh" status 2>/dev/null || "$SCRIPT_DIR/edtime-sync.sh" status
    ;;
  *)
