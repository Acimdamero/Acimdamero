...
EDTIME_MAPPING="${EDTIME_MAPPING:-$HUB_HOME/edtime-mapping.json}"
EDTIME_DRIVE_FOLDER="${EDTIME_DRIVE_FOLDER:-Automation Hub/Edtime}"
...
edtime_resolve_drive_base() {
  local sub="${1:-}"
  local base
  base="$(resolve_backup_dest | sed 's|/Backups$||')"
  if [[ -z "$base" || "$base" == "$HUB_HOME/backups" ]]; then
