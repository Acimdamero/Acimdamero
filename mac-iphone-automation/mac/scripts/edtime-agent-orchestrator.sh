...
# Dipanggil oleh run-edtime-agentic.sh dan LaunchAgent edtime.
...
load_secrets() {
  local f
  for f in "$HUB_HOME/secrets.env" "$REPO_DIR/config/secrets.env"; do
    if [[ -f "$f" ]]; then
      set -a
...
phase_install() {
  write_report "install" "running" "Installing hub + edtime scripts"
  bash "$REPO_DIR/mac/install.sh"
  bash "$REPO_DIR/setup-edtime-sync.sh" --non-interactive
  write_report "install" "ok" "Hub and edtime scripts installed"
}
...
phase_enable_autostart() {
  ...
  write_report "autostart" "running" "Loading LaunchAgents"
  local LA="$HOME/Library/LaunchAgents"
  mkdir -p "$LA"
  sed "s|__HUB_HOME__|$HUB_HOME|g" "$REPO_DIR/mac/launchd/com.automation.hub.plist.template" \
    > "$LA/com.automation.hub.plist"
  sed -e "s|__HUB_HOME__|$HUB_HOME|g" -e "s|__REPO_DIR__|$REPO_DIR|g" \
    "$REPO_DIR/mac/launchd/com.automation.edtime-sync.plist.template" \
    > "$LA/com.automation.edtime-sync.plist"
  launchctl unload "$LA/com.automation.edtime-sync.plist" 2>/dev/null || true
  launchctl load "$LA/com.automation.edtime-sync.plist" 2>/dev/null || true
  write_report "autostart" "ok" "Daemon hub + edtime scheduled"
}
...
phase_notify() {
  ...
  if [[ -x "$HUB_HOME/run-task.sh" ]] && [[ "${SKIP_WAHA:-0}" != "1" ]]; then
    "$HUB_HOME/run-task.sh" waha-send-name "${NOTIFY_CONTACT:-}" "edtime sync agent selesai ✅" 2>/dev/null || true
  fi
}
...
hub = os.path.expanduser("~/.automation-hub")
decisions = []
...
webhook = os.popen(
    "security find-generic-password -s automation-hub -a webhook-url -w 2>/dev/null"
).read().strip()
need(not webhook, "deploy_apps_script",
     "Deploy Apps Script + isi HUB_WEBHOOK_URL di secrets.env",
     "Extensions→Apps Script→Deploy Web App→bash run-edtime-agentic.sh")
...
out = {
    "updated_at": datetime.now(timezone.utc).isoformat(),
    "decisions": decisions,
    "monitor_command": "~/.automation-hub/scripts/edtime-monitor.sh",
}
