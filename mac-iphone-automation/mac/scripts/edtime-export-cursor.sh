...
Output:
  Google Drive/Automation Hub/Edtime/cursor/latest.json
  ~/.automation-hub/edtime/cursor-latest.json (salinan lokal)
EOF
}
...
bundle = {
    ...
    "meta": meta,
    "cursor_hints": {
        "read_via": "Google Drive MCP → Automation Hub/Edtime/cursor/latest.json",
...
if [[ ! -f "$LOCAL_CACHE" ]]; then
  ...
  echo '{"schema":"automation-hub/edtime-cursor-export/v1","schedule":[],"meta":{}}' > "$LOCAL_CACHE"
fi
