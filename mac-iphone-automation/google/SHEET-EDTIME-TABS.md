...
Dibuat otomatis oleh `EdtimeSync.gs` — baris datar untuk Cursor baca.
...
1. Paste `QueueSync.gs` + `EdtimeSync.gs` + `WhatsAppInbound.gs` (opsional)
2. Deploy Web App → salin URL ke Mac:

```bash
bash setup-edtime-sync.sh
# atau:
~/.automation-hub/scripts/edtime-save-credentials.sh webhook 'YOUR_WEB_APP_URL'
```
